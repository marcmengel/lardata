...
namespace trkf {
  ...
  TrackState TrackStatePropagator::rotateToPlane(bool& success, const TrackState& origin, const Plane& target, double& dw2dw1) const {
  ...
  double TrackStatePropagator::distanceToPlane(bool& success, const Point_t& origpos, const Vector_t& origmom, const Plane& target) const {
  ...
  std::pair<double,double> TrackStatePropagator::distancePairToPlane(bool& success, const Point_t& origpos, const Vector_t& origmom, const Plane& target) const {
    const Point_t& targpos = target.position();
    const Vector_t& targdir = target.direction();
    //check that origmom is not along the plane, i.e. targdir.Dot(origmom.Unit())=0
    if (targdir.Dot(origmom.Unit())==0) {
