...
namespace trkf {
  ...
  bool InteractPlane::noise(const KTrack& trk, double s, TrackError& noise_matrix) const
  {
    ...
    return noise(dudw, dvdw, pinv, mass, s, (trk.getDirection() == Surface::BACKWARD), noise_matrix);
  }


  template<typename T> bool InteractPlane::noise(double dudw, double dvdw, double pinv, double mass, double s, bool flipSign, T& noise_matrix) const {
    ...
    // double theta_fact = (0.0136 * pinv * betainv) * (1. + 0.038 * std::log(std::abs(s/x0)));//fixme
  ...
  /*  
  bool InteractPlane::noise(double dudw, double dvdw, double pinv, double mass, double s, bool flipSign, TrackError& noise_matrix) const {
    // If distance is zero, or momentum is infinite, return zero noise.
  ...
  bool InteractPlane::noise(double dudw, double dvdw, double pinv, double mass, double s, bool flipSign, recob::tracking::SMatrixSym55& noise_matrix) const {
    // If distance is zero, or momentum is infinite, return zero noise.
