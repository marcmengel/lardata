...
namespace util {
  ...
  //------------------------------------------------------------
  double
  DetectorPropertiesServiceArgoNeuT::ConvertTDCToTicks(double tdc) const
  {
  ...
  //--------------------------------------------------------------
  double
  DetectorPropertiesServiceArgoNeuT::ConvertTicksToTDC(double ticks) const
  {
  ...
  //--------------------------------------------------------------------
  void
  DetectorPropertiesServiceArgoNeuT::reconfigure(fhicl::ParameterSet const& p)
  {
    ...
    if (p.get_if_present<bool>("InheritTriggerOffset", b))
      throw cet::exception(__FUNCTION__) << "InheritTriggerOffset is a deprecated fcl parameter "
                                            "for DetectorPropertiesServiceArgoNeuT!";
  ...
  //-------------------------------------------------------------
  void
  DetectorPropertiesServiceArgoNeuT::preProcessEvent(const art::Event& evt, art::ScheduleContext)
  {
  ...
  //---------------------------------------------------------------------------------
  void
  DetectorPropertiesServiceArgoNeuT::checkDBstatus() const
  {
    bool fToughErrorTreatment =
      art::ServiceHandle<util::DatabaseUtil const>()->ToughErrorTreatment();
    ...
    if (!fAlreadyReadFromDB && fToughErrorTreatment && fShouldConnect)
      throw cet::exception("DetectorPropertiesServiceArgoNeuT")
        << " Extracting values from DetectorPropertiesServiceArgoNeuT before they "
    ...
    //Have not read from DB, should read and requested soft treatment
      ...
      mf::LogWarning("DetectorPropertiesServiceArgoNeuT")
        << "!!! Extracting values from DetectorPropertiesServiceArgoNeuT before they "
  ...
  //------------------------------------------------------------------------------------//
  int
  DetectorPropertiesServiceArgoNeuT::TriggerOffset() const
  {
    return fTPCClock.Ticks(lar::providerFrom<detinfo::DetectorClocksService>()->TriggerOffsetTPC() *
                           -1.);
  }
  ...
  double
  DetectorPropertiesServiceArgoNeuT::ConvertXToTicks(double X, int p, int t, int c) const
  {
  ...
  double
  DetectorPropertiesServiceArgoNeuT::ConvertTicksToX(double ticks, int p, int t, int c) const
  {
    if (!fXTicksParamsLoaded) CalculateXTicksParams();
  ...
  void
  DetectorPropertiesServiceArgoNeuT::CalculateXTicksParams() const
  {
...
For plane = 0, t offset is pitch/Coeff[1] - (pitch+xyz[0])/Coeff[0]
  ...
  double
  DetectorPropertiesServiceArgoNeuT::GetXTicksCoefficient(int t, int c) const
  {
    if (!fXTicksParamsLoaded) CalculateXTicksParams();
  ...
  double
  DetectorPropertiesServiceArgoNeuT::GetXTicksCoefficient() const
  {
    if (!fXTicksParamsLoaded) CalculateXTicksParams();
  ...
  double
  DetectorPropertiesServiceArgoNeuT::GetXTicksOffset(int p, int t, int c) const
  {
    if (!fXTicksParamsLoaded) CalculateXTicksParams();
  ...
  DetectorPropertiesServiceArgoNeuT::postOpenFile(const std::string& filename)
  {
    ...
    if (filename.size() != 0) {
      ...
      if (file != 0 && !file->IsZombie() && file->IsOpen()) {
        ...
        if (fInheritNumberTimeSamples && nNumberTimeSamples != 0 &&
...
namespace util {
  ...
  DEFINE_ART_SERVICE_INTERFACE_IMPL(DetectorPropertiesServiceArgoNeuT,
