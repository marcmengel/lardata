...
namespace lar {
  namespace util {
    ...
    namespace details {
       ...
       * @todo can be generalized to a power N
       * 
       * 
       */
      ...
      class DataTracker2 {
        ...
        /// Adds the specified weight to the statistics
        void add(Data_t v, Weight_t w) { sums[0] += w*v; sums[1] += w*v*v; }
        
        /// Resets the count
        void clear() { sums.fill(Data_t(0)); }
        
        /// Returns the weighted sum of the entries
        Weight_t Sum() const { return sums[0]; }
        
        /// Returns the weighted sum of the square of the entries
        Weight_t SumSq() const { return sums[1]; }
        
        /// Returns the sum of the values to the power N (1 <= N <= 2)
        template <unsigned int N>
        Weight_t SumN() const
          {
            static_assert((N > 0) && (N <= 2), "Invalid sum of power requested");
            return sums[N - 1];
          } // SumN()
        
          protected:
        std::array<Weight_t, 2> sums;
