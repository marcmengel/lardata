...
void MatrixTest() {
  ...
  BOOST_CHECK_EQUAL(indices.minor().rank(), 1U);
  BOOST_CHECK_EQUAL(indices.minor().size(), 3U);
...
void TensorRank3Test() {
  ...
  BOOST_CHECK_EQUAL(indices.minor().rank(), 2U);
  BOOST_CHECK_EQUAL(indices.minor().size(), 3U * 4U);
